...
1. [証拠動画](証拠動画ガイドライン)のビットレート等について制限はありませんが、第三者による視聴が困難である場合はそれを証拠動画として認めません。
1. 各レギュレーションで特に認められた場合を除き、すべてのランキングで投稿時点で１位になることが見込まれるスコアには証拠動画が必要です。
   ただし以下に定める特例に該当する場合は免除される場合があります。
1. [証拠画像](証拠画像ガイドライン)はリザルト画面全体を撮ることを原則とし、スコアや必要な情報の一部が確認できない場合はそれを証拠画像として認めません。
...
1. すべてのユーザーが豊かに活動するための実践方針として、当サイトでは[[相互承認の基本理念]]を掲げ、ユーザーに実践を促すこととします。
...
1. 当サイトでは、特定商品の宣伝やスパム、アフィリエイトなどを目的とするスクリーンネーム、ハンドルネームの登録は禁止します。
...
1. 最新年ランキングに掲載されている記録は、投稿時点で公開設定にしていた証拠動画を、投稿後に動画サイト側で非公開にする行為は認めません。
...
1. 当サイトのコンテンツのうち、一般公開されていない情報（ボイスチャンネルでのみ語られた個人情報を含む）を管理人の許可を得ずにSNS等に掲載することは絶対に禁止します。
...
1. 公序良俗に反する内容・発言（ボイスチャットにおける発言を含む）等については、本条の「B 投稿」第５項以降に定める禁止事項の一切はDiscordでも禁止します。
...
1. Discordにおいて、本条「F 禁止事項の例外規定」の条件を満たすユーザーがピクミン以外のゲーム等の配信をする場合、配信する場合は最大限節度を守った上で配信し、かつ可能なかぎり「カメラ機能」を利用した配信は控えてください（「画面共有機能」を利用してください）。
...
1. Discordにおいて、配信プレビューを非表示にする行為を禁止します。ただし、サーバーブースターはこの限りではありません。

#### F 禁止事項の例外規定

ピクミン以外のゲーム等を配信できるのは以下のすべての条件に合致するユーザー、またはサーバーブースト提供者とします。
この規定は、チャンネルの種別を問わず適用され、配信の都度条件を満たしていなければなりません。
条件を満たしているかどうかは自己申告制となり、証拠の提出は不要です。
なお、その場の状況から配信するのが妥当と客観的に判断できる配信については、この条件を満たす必要はありません。
管理人または管理人に委任された者から個別にピクミン以外のゲーム等の配信を禁止されているユーザーについては、
サーバーブースト状況や以下条件を満たしているかどうかにかかわらず配信できません。

1. サーバーに参加してから１年が経過している。
1. 直近２週間以内に、本人の他に２人以上参加者がいるボイスチャンネル内で120分以上のピクミン配信を行った。
...
1. 今後永続的または一時的な画面共有の利用またはボイスチャンネルへの参加の禁止。
