...
export default function Difficulties({users, stage, rule, consoles, year, difficulty}){
    ...
    return (
        <Grid className="console-wrapper" item key={difficulty}>
            <Box className={`console-box`}
                     component={Link}
                     href={'/stage/'+stage+'/'+consoles+'/'+rule+'/'+year+'/'+difficulty}>
                {t.difficulty[post.difficulty]}<br/>
