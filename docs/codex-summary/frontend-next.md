...
- `/total/10`、`/total/20`、`/total/30`、`/total/40` のシリーズトップだけ、通常の `StageList` 直下に RTA、その他の順で混在表示する `SpecialStages` を追加する。1行6件で折り返し、スマホ幅では6件分の幅を横スクロールする。
- `SpecialStages` 自身が6列の CSS Grid を持ち、総合、900番台ステージ、Speedrunページのいずれでも親コンポーネントに依存せず1行6件で表示する。ラベルに全角括弧 `（` がある場合はその直前で改行する。
